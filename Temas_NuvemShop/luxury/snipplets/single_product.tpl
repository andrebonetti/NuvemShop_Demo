<div class="dest-gral {% if product.images[0].id == 0 %}no-image{% endif %}">
    <div class="head">
        ...
        {% endif %}
        <a href="{{ product.url }}?variant={{ product.selected_or_first_available_variant.id }}" title="{{ product.name }}" class="product-image{% if not product.available %}out-stock-img{% endif %}">
        	{% if not product.available %}
    ...
    <div class="bajada" itemscope itemtype="http://schema.org/Product" >
        ...
        {% endif %}
        <div class="title">
        	<h3 itemprop="name"><a href="{{ product.url }}?variant={{ product.selected_or_first_available_variant.id }}" title="{{ product.name }}">{{ product.name }}</a></h3>
        </div>
