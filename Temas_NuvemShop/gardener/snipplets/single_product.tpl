<div class="single-product" itemscope itemtype="http://schema.org/Product">
	...
	<figure>
		<a itemprop="url" href="{{ product.url }}?variant={{ product.selected_or_first_available_variant.id }}" title="{{ product.name }}">{{ product.featured_image | product_image_url("medium") | img_tag(product.name) }}</a>
        ...
        <figcaption>
            ...
            <h3 itemprop="name" {% if new_font_size %}style="font-size: {{ new_font_size }}px"{% endif %}><a href="{{ product.url }}" title="{{ product.name }}">{{ product.name }}</a></h3>
            ...
            <div class="price-container {% if not product.display_price%}hidden{% endif %}" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
            	<a href="{{ product.url }}?variant={{ product.selected_or_first_available_variant.id }}" title="{{ product.name }}">
