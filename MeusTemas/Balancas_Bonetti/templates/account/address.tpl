            ...
            <div class="account-form-wrapper span6 offset3 sst">
                <form action="" method="post" class="form-horizontal">
                    ...
                    {% if result.errors.address %}
                        <div class="st alert alert-error c">{{ 'Debes ingresar tu dirección!' | translate }}</div>
                    {% endif %}
                    {% if result.errors.number %}
                        <div class="st alert alert-error c">{{ 'Debes ingresar tu número!' | translate }}</div>
                    {% endif %}
                    {% if result.errors.floor %}
                        <div class="st alert alert-error c">{{ 'Debes ingresar tu piso!' | translate }}</div>
                    {% endif %}
                    {% if result.errors.locality %}
                        <div class="st alert alert-error c">{{ 'Debes ingresar tu localidad!' | translate }}</div>
                    {% endif %}
                    {% if result.errors.zipcode %}
                        <div class="st alert alert-error c">{{ 'Debes ingresar tu código postal!' | translate }}</div>
                    {% endif %}
                    {% if result.errors.city %}
                        <div class="st alert alert-error c">{{ 'Debes ingresar tu ciudad!' | translate }}</div>
                    {% endif %}
                    {% if result.errors.province %}
                        <div class="st alert alert-error c">{{ 'Debes ingresar tu provincia!' | translate }}</div>
                    {% endif %}
                    {% if result.errors.country %}
                        <div class="st alert alert-error c">{{ 'Debes ingresar tu país!' | translate }}</div>
                    {% endif %}
                    {% if result.errors.phone %}
                        <div class="st alert alert-error c">{{ 'Debes ingresar tu teléfono!' | translate }}</div>
                    {% endif %}
